...
\subsection{Complejidad}
Dentro del ciclo principal, las operaciones de elección de movimiento, \texit{Agregar} e \texit{Intercambiar}, tienen complejidad O($N^2 * log N$) y O($N$) para \texit{Eliminar}. La aplicación de la operación \textit{Eliminar} tiene una complejidad de O($N$), obteniendose una complejidad de O($2*N + 2*(N^2 * log N)$) = O($N^2 * log N$).
